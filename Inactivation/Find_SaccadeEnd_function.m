function [x_endall, y_endall, condition, RT, meanx, meany] = Find_SaccadeEnd_function(filename)
...
for j = 1:length(ntr)
    ...
    if isempty(test_time)
        ...
        condition(j) = beh(ntrt).Condition;
    end
    ...
    for j = 1:8
        x_all = [];
        y_all = [];
        for h = 1:length(x_endall)
            if condition(h) == j
                x_all = [x_all x_endall(h)];
                y_all = [y_all y_endall(h)];
            end
            meanx(j) = nanmean(x_all);
            meany(j) = nanmean(y_all);
        end
    end
